...
$(function() {
    ...
    var config = {
        ...
        NumberOfYears: 2,
        MergeYears: false // Combine data from every year & create an average / number of years
     };
                    ...
                    if (config.MergeYears) {
                        ...
                        if (!found_team) {
                            matches.push({
                                ...
                                year: Team.url.substring(Team.url.length - 4) == '2015' ? '2015' : '2016'
                            });
                        }
                    } else {
                        var year_data = Team.url.substring(Team.url.length - 4) == '2015' ? '2015' : '2016';
    ...
    function searchArray(array, objectName, searchTerm) {
        ...
        if (result.length === 0) { // not found
          ...
          return result[0]; // Take first result // 'ERROR: 1+ items found';
        }
    }
    ...
    function displayFixtures(fs) {
        ...
        for(var i = 0; i < fs.length; i++) {
            ...
            output.append($('<p></p>').text('Fixture estimates based on 2015-16 data (not 2014-15).'));
            ...
            for(var j = 0; j < fs[i].matches.length; j++) {
                ...
                var crColour = 'default-colour';
                if (game.correctResult == 'Y') {
                    crColour = 'success-colour';
                }
                ...
                var tableRow = $('<tr></tr>').append(
                    ...
                    $('<td>').text(game.correctResult).attr('class', crColour),
            ...
            table.append(
                $('<tr></tr>').append(
                    ...
                    $('<td>').text(''),
                    $('<td>').text('Success'),
                    // $('<td>').text((TotalCorrectScore / fs[i].matches.length).toFixed(1)),
                    $('<td>').text((TotalCorrectResult / fs[i].matches.length).toFixed(1)),
                    $('<td>').text((TotalCorrectOverOrUnder / fs[i].matches.length).toFixed(1))
                )
            );
